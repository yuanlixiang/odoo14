...
/** Mixin按照父子关系来构造对象的生命周期。每个对象都有一个父级和多个子级。
当某对象被销毁时，所有子级同时被销毁。当一个对象被销毁时，它的所有子对象也被销毁，释放它们之前可能保留的任何资源
...
var ParentedMixin = {
     ...
     * getChildren() method is called.
     设置当前对象的父对象。
     当调用此方法时，父对象也将得到通知，并在调用getChildren()方法时返回当前对象。
     如果当前对象已经有一个父对象，那么它之前就会被注销，这意味着在调用其getChildren()方法时，前一个父对象将不再返回当前对象。
 ...
 *   这个类只处理事件分派，它不需要扩展，也不需要直接使用。
 所有与父级和自动注销事件的集成都是在EventDispatcherMixin中完成的
 ...
 *包含事件系统的Mixin。事件还可以通过指定目标对象(在引发事件时接收该事件的对象)来注册。事件发送对象和目标对象彼此存储或引用。
 当任何对象被销毁时(当调用ParentedMixin的destroy()方法时)，这用于正确地删除对事件处理程序的所有引用。
 为了避免内存泄漏和幻像事件(引发并发送给先前已销毁的对象的事件)，必须删除这些引用。
