 ...
 * BasicView, so, they can work with the records obtained from a BasicModel.
 ...
 该文件包含大多数的基本字段挂件，这些字段挂件应该被继承自BasicView 的视图使用。
 * 所以它们可以处理从BasicModel获得的记录。
 */

var AbstractField = require('web.AbstractField');
var config = require('web.config');   // 12版本中没有
var core = require('web.core');
var datepicker = require('web.datepicker');
var deprecatedFields = require('web.basic_fields.deprecated');  // 12版本中没有
var dom = require('web.dom');
var Domain = require('web.Domain');
var DomainSelector = require('web.DomainSelector');
var DomainSelectorDialog = require('web.DomainSelectorDialog');
var framework = require('web.framework');
var py_utils = require('web.py_utils');  // 12版本中没有
var session = require('web.session');
var utils = require('web.utils');
var view_dialogs = require('web.view_dialogs');
var field_utils = require('web.field_utils');
var time = require('web.time');  // 12版本中没有
...
var DebouncedField = AbstractField.extend({
     ...
     * the suggested value.
     对于反复快速变化的字段挂件，用DEBOUNCE 属性来防抖动变化是一个好主意。
     * 该防抖动的事件间隔建议为1000000000，即十亿毫秒。
     */
    DEBOUNCE: 1000000000,
    ...
    init: function () {
        ...
        // _isDirty属性用于检测用户至少与挂件进行了一次交互，
        // 因此，如果用户未进行任何更改，我们可以防止它触发commitChanges中的field_changed
        this._isDirty = false;
        if (this.mode === 'edit') {
            if (this.DEBOUNCE) {
     ...
     * be done immediately.
     *字段的主要动作是防抖动并且设置字段的值。当字段的更改请求提交时，则防抖动动作不能不被立即执行（即不再做防抖动）
     * @override
     */
     ...
     * 该方法用于通知外部世界挂件的最新值，该方法有一个再init方法中自动创建的名为“_doDebouncedAction”的防抖动版本。
     * @private
     */
    _doAction: function () {
